...
def load_data_tc_nowcasting(data, config):
    ...
    sampled_data = np.zeros((data.shape[0] - (config.num_timesteps - 1),config.num_timesteps + 1,data.shape[1],data.shape[2], data.shape[3]))

    def get_gaussian_probs(latitude, longitude, config):
        gaussian_radius = config.radius
        clip_prob = 0.1
        x = np.arange(0, 41, 1)
        y = np.arange(0, 161, 1)
    ...
    for i in range(config.num_timesteps - 1, data.shape[0]):
        k = 0
        while k < sampled_data.shape[1] - 1:
            sampled_data[i - (config.num_timesteps - 1), k, :, :, :] = reshaped_data[i - (sampled_data.shape[1] - 2) - k, 0, :, :, :]
            k += 1
        if tc_labels[i] == 1:
            ...
            for (lat,lon) in locations[i]:
                lat = lat - offset[0]
                lon = lon - offset[1]
                gaussian_probs = get_gaussian_probs(lat, lon, config)
                label_grid = label_grid + gaussian_probs
            sampled_data[i - (config.num_timesteps - 1), k, :, :, :] = label_grid
        else:
            label_zeros = np.zeros((data.shape[1], data.shape[2], data.shape[3]))
            sampled_data[i - (config.num_timesteps - 1), k, :, :, :] = label_zeros
            
    del reshaped_data
    labels = tc_labels[config.num_timesteps - 1:]
    print(len(labels))
    ...
    train_data, train_target, train_output_frames, train_grid_labels = prepare_test_data_nowcasting(train_sampled_data, train_labels, config)
    
    test_sampled_data = sampled_data[int(sampled_data.shape[0] * 0.8):]
    test_labels = labels[int(sampled_data.shape[0] * 0.8):]
    test_data, test_target, test_output_frames, test_grid_labels = prepare_test_data_nowcasting(test_sampled_data, test_labels, config)
...
def prepare_test_data_nowcasting(sampled_data, labels, config):
    ...
    sampled_target_data = np.zeros((sampled_data.shape[0], 1, sampled_data.shape[2], sampled_data.shape[3], sampled_data.shape[4]))
    ...
    for i in range(sampled_data.shape[0]):
        ...
        while k < config.num_timesteps:
            ...
            k += 1
        sampled_target_data[i, 0, :, :, :] = sampled_data[i, k - 1, :, :, :]
