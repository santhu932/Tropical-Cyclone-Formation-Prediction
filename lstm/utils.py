...
def data_preprocessing_forecasting_new1(data, config):
    random.seed(config.s)
    dataframe = pd.read_csv('tc_0h.csv')
    offset = (5, 100)
...
def data_preprocessing_forecasting_new2(data, config):
    random.seed(config.s)
    dataframe = pd.read_csv('tc_0h.csv')
    ...
    data = data.numpy()
    reshaped_data = data.reshape(data.shape[0],1, data.shape[1],data.shape[2], data.shape[3])
    sampled_data = np.zeros((len(indices),8,data.shape[1],data.shape[2], data.shape[3]))
...
#    example_target[0,1, :, :, :] = reshaped_data[51, 0, :, :, :]
    ...
    for i , val in enumerate(indices):
        sampled_data[i,0, :, :, :] = reshaped_data[val - 4, 0, :, :, :]
        sampled_data[i,1, :, :, :] = reshaped_data[val - 3, 0, :, :, :]
        sampled_data[i,2, :, :, :] = reshaped_data[val - 2, 0, :, :, :]
        sampled_data[i,3, :, :, :] = reshaped_data[val - 1, 0, :, :, :]
        sampled_data[i,4, :, :, :] = reshaped_data[val - 0, 0, :, :, :]
        sampled_data[i,5, :, :, :] = reshaped_data[val + 1, 0, :, :, :]
        sampled_data[i,6, :, :, :] = reshaped_data[val + 2, 0, :, :, :]
        sampled_data[i,7, :, :, :] = reshaped_data[val + 3, 0, :, :, :]
    ...
    sampled_input_data = np.zeros((sampled_data.shape[0], 4, sampled_data.shape[2], sampled_data.shape[3], sampled_data.shape[4]))
    sampled_target_data = np.zeros((sampled_data.shape[0], 4, sampled_data.shape[2], sampled_data.shape[3], sampled_data.shape[4]))
    ...
    for i in range(sampled_data.shape[0]):
        ...
        sampled_target_data[i,0, :, :, :] = sampled_data[i,4, :, :, :]
        sampled_target_data[i,1, :, :, :] = sampled_data[i,5, :, :, :]
        sampled_target_data[i,2, :, :, :] = sampled_data[i,6, :, :, :]
        sampled_target_data[i,3, :, :, :] = sampled_data[i,7, :, :, :]
    del sampled_data

    augmented_input_data = np.zeros((augmented_data.shape[0], 4, augmented_data.shape[2], augmented_data.shape[3], augmented_data.shape[4]))
    augmented_target_data = np.zeros((augmented_data.shape[0], 4, augmented_data.shape[2], augmented_data.shape[3], augmented_data.shape[4]))
    for i in range(augmented_data.shape[0]):
        augmented_input_data[i,0, :, :, :] = augmented_data[i,0, :, :, :]
        augmented_input_data[i,1, :, :, :] = augmented_data[i,1, :, :, :]
        augmented_input_data[i,2, :, :, :] = augmented_data[i,2, :, :, :]
        ...
        augmented_target_data[i,0, :, :, :] = augmented_data[i,4, :, :, :]
        augmented_target_data[i,1, :, :, :] = augmented_data[i,5, :, :, :]
        augmented_target_data[i,2, :, :, :] = augmented_data[i,6, :, :, :]
        augmented_target_data[i,3, :, :, :] = augmented_data[i,7, :, :, :]
    del augmented_data
