...
class Seq2Seq(nn.Module):
    ...
    activation, frame_size, num_layers, num_timesteps, bias, w_init, hidden_size):
        ...
        self.fc1 = nn.Linear(frame_size[0] * frame_size[1], 1024, bias = self.bias)
...
#        )
    ...
    def forward(self, X, target_frames = None, prob_mask = None, prob_mask1 = None, prediction = False):
        ...
        #temp = output.clone()
        recon_frames = torch.zeros(recon_frame.shape[0], 4, recon_frame.shape[1], recon_frame.shape[2], recon_frame.shape[3], device = device)
        ...
        for t in range(self.num_timesteps - 1):
            ...
            recon_frames[:,t + 1] = recon_frame.clone()
            if prediction == False:
